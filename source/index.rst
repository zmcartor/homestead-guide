.. homestead guide documentation master file, created by
   sphinx-quickstart on Tue Jan  5 17:30:05 2016.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

Expanse Homestead Documentation
===============================================================================

..  image:: img/expanse-homestead-documentation-logo.png
..   :height: 500px
..   :width: 394 px
   :scale: 50 %
   :alt: expanse-logo
   :align: center

This documentation is the result of an ongoing collaborative effort by
volunteers from the Expanse :ref:`community`.  Although it has not been
authorized by the :ref:`foundation`, we hope you will find it useful,
and welcome new :ref:`contributors`.


Contents
===============================================================================

.. toctree::
   :maxdepth: 4

   introduction/index.rst
   expanse-clients/index.rst
   account-management.rst
   ether.rst
   network/index.rst
   mining.rst
   contracts-and-transactions/index.rst
   frequently-asked-questions/frequently-asked-questions.rst
   glossary.rst
   about.rst


Improve the Documentation
===============================================================================

See `this page <https://expanse-homestead.readthedocs.org/en/latest/about.html>`__ to help us improve the documentation.
