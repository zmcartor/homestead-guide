...
Below is an excerpt from the blog post `On Public and Private Blockchains <https://blog.expanse.org/2015/08/07/on-public-and-private-blockchains/>`_ that explains the difference between the three types of blockchains based on permissioning:
...
NOTE: ``--fast`` can only be run if you are syncing your blockchain from scratch and only the first time you download the blockchain for security reasons. `See this Reddit post for more information <https://www.reddit.com/r/expanse/comments/3y9316/geth_fast_option_question/>`_.
